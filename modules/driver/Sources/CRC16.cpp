...
namespace DRIVER {
    ...
    void addCRC16(unsigned char *msg) {
        if (!msg)
            return;
        Append_CRC16_Check_Sum((uint8_t *)msg, sizeof(SerialWriteData));
    }

    bool verifyCRC16(SerialReadData::RobotStatus *data) {
        if (!data)
            return false;
        return Verify_CRC16_Check_Sum((uint8_t *)data, sizeof(SerialReadData::RobotStatus));
    }

    bool verifyCRC16(SerialReadData::IMU_Flag *data) {
        if (!data)
            return false;
        return Verify_CRC16_Check_Sum((uint8_t *)data, sizeof(SerialReadData::IMU_Flag));
    }
}