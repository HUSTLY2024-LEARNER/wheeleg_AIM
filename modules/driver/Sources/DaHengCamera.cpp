...
namespace DRIVER
{
    ...
    /// Initialize libary
    GX_STATUS DaHengCamera::initLib()
    {
    ...
    GX_STATUS DaHengCamera::openDevice(const char *CameraSN)
    {
    ...
    void
    DaHengCamera::setExposureGainParam(bool AutoExposure, bool AutoGain, double ExposureTime, double AutoExposureTimeMin,
                                   double AutoExposureTimeMax, double Gain, double AutoGainMin, double AutoGainMax,
                                   int64_t GrayValueMin, int64_t GrayValueMax, bool TRIGGER_SOURCE_LINE2)
    {
        ...
        exposure_gain.m_i64GrayValueMin = GrayValueMin;
        exposure_gain.m_i64GrayValueMax = GrayValueMax;
        exposure_gain.m_TRIGGER_SOURCE_LINE2 = TRIGGER_SOURCE_LINE2;
    }

    /// Set camera exposure and gain
    GX_STATUS DaHengCamera::setExposureGain()
    {
        GX_STATUS status;
        ...
        status = GXSetInt(g_hDevice, GX_INT_GRAY_VALUE, exposure_gain.m_i64GrayValueMin);
        GX_VERIFY(status);

        status = GXSetInt(g_hDevice, GX_INT_GRAY_VALUE, exposure_gain.m_i64GrayValueMax);
        GX_VERIFY(status);
    ...
    void DaHengCamera::setRoiParam(int64_t Width, int64_t Height, int64_t OffsetX, int64_t OffsetY)
    {
    ...
    void DaHengCamera::setAAROIParam(int64_t Width, int64_t Height, int64_t OffsetX, int64_t OffsetY)
    {
        aaroi.m_i64_AAROI_Height = Height;
        aaroi.m_i64_AAROI_Width = Width;
        aaroi.m_i64_AAROI_OffsetX = OffsetX;
        aaroi.m_i64_AAROI_OffsetY = OffsetY;
    }

    /// Set camera roi
    GX_STATUS DaHengCamera::setRoi()
    {
        GX_STATUS status = GX_STATUS_SUCCESS;
    ...
    void DaHengCamera::setWhiteBalanceParam(bool WhiteBalanceOn, float BalanceRatioRed, float BalanceRatioBlue, float BalanceRatioGreen, GX_AWB_LAMP_HOUSE_ENTRY lightSource)
    {
        // 自动白平衡光照环境
        ...
        white_balance.balanceRatioRed = BalanceRatioRed;
        white_balance.balanceRatioBlue = BalanceRatioBlue;
        white_balance.balanceRatioGreen = BalanceRatioGreen;
    }

    GX_STATUS DaHengCamera::setAAROI()
    {
        GX_STATUS status;
        status = GXSetInt(g_hDevice, GX_INT_AAROI_WIDTH, aaroi.m_i64_AAROI_Width);
        status = GXSetInt(g_hDevice, GX_INT_AAROI_HEIGHT, aaroi.m_i64_AAROI_Height);
        status = GXSetInt(g_hDevice, GX_INT_AAROI_OFFSETX, aaroi.m_i64_AAROI_OffsetX);
        status = GXSetInt(g_hDevice, GX_INT_AAROI_OFFSETY, aaroi.m_i64_AAROI_OffsetY);
        GX_VERIFY(status);
    ...
    GX_STATUS DaHengCamera::setWhiteBalance()
    {
        // 选择白平衡通道
        GX_STATUS status;
        ...
        else
        {
            ...
            status = GXSetFloat(g_hDevice, GX_FLOAT_BALANCE_RATIO, white_balance.balanceRatioRed);

            status = GXSetEnum(g_hDevice, GX_ENUM_BALANCE_RATIO_SELECTOR, GX_BALANCE_RATIO_SELECTOR_BLUE);

            status = GXSetFloat(g_hDevice, GX_FLOAT_BALANCE_RATIO, white_balance.balanceRatioBlue);

            status = GXSetEnum(g_hDevice, GX_ENUM_BALANCE_RATIO_SELECTOR, GX_BALANCE_RATIO_SELECTOR_GREEN);

            status = GXSetFloat(g_hDevice, GX_FLOAT_BALANCE_RATIO, white_balance.balanceRatioGreen);
    ...
    GX_STATUS DaHengCamera::acquisitionStart()
    {
        //////////////////////////////////////////SOME SETTINGS/////////////////////////////////////////////
...
*/
    //-------------------------------------------------
    void *DaHengCamera::ProcGetImage(Mat *image, std::chrono::steady_clock::time_point *time_stamp)
    {
        ...
        {
            *threadParam->m_pTimeStamp = std::chrono::steady_clock::now(); // 获取当前时间
            ...
            {
                (*image).create(pFrameBuffer->nHeight, pFrameBuffer->nWidth, CV_8UC3);
                ...
                emDXStatus = DxRaw8toRGB24((unsigned char *)pFrameBuffer->pImgBuf, (*image).data, pFrameBuffer->nWidth,
                ...
                {
                    // memcpy((*image).data, pBGRBuf, pFrameBuffer->nHeight * pFrameBuffer->nWidth * 3);
                    // // producer 获取Mat图像接口
                    // delete[] pBGRBuf;
                    // pBGRBuf = NULL;
                }
